...
function test_largescale_env()
    ...
    savefig(p, "data/state.pdf")
    savefig(p2, "data/state2.pdf")
    savefig(env0p, "data/env0state.pdf")
    savefig(env0p2, "data/env0state2.pdf")
end

function test_custom_env()
    print_msg("myenv (use of custom env)")
    log_dir = "data"
    file_name = "test.h5"
    logger = Logger(log_dir=log_dir, file_name=file_name)
